...
mkProp ty =
  ...
  prefixProp r f xs
    ...
    | i == indexIdent,     [x] <- xs = return [CLocated (CIndex x) r]
    | i == literalIdent, [x,y] <- xs = return [CLocated (CLiteral x y) r]
    | otherwise                      = return [CLocated (CType (TUser f xs)) r]
    where
    i = getIdent f

zeroIdent, logicIdent, arithIdent, finIdent, cmpIdent,
  signedCmpIdent, indexIdent, literalIdent :: Ident
